...
window.onload = async () => {
  try {
    await fetch("http://localhost:8080/list-todos")
      .then((allToDo) => {
        return allToDo.json();
      })
      .then((datas) => {
...
async function changePosition(currEl, currElId) {
  ...
  try {
    ...
    if (currEl.previousSibling !== null) {
      ...
      await fetch("http://localhost:8080/read-todos/" + prevElId)
        .then((data) => {
          return await data.json();
        })
        .then((json) => {
          prevElIndexNumber = json.data[0].index_number;
        });
    }
    ...
    if (currEl.nextSibling != null) {
      ...
      await fetch("http://localhost:8080/read-todos/" + nextElId)
        .then((data) => {
          return await data.json();
        })
        .then((json) => {
          nextElIndexNumber = json.data[0].index_number;
        });
    }
    ...
    const updateUrl = "http://localhost:8080/order-todos/" + currElId;
...
async function finishEditToDo(button, input, id) {
  ...
  const updateUrl = "http://localhost:8080/edit-todos/" + id;
  await fetch(updateUrl, {
    method: "POST",
    headers: {
      "Content-type": "application/json",
...
async function deleteToDo(node, id) {
  ...
  const deleteUrl = "http://localhost:8080/delete-todos/" + id;
